...
func (c *Consumer) consume(channel *amqp.Channel, id int) {
	...
	for msg := range msgs {
		...
		var messageBody model.Email
		if err := json.Unmarshal(msg.Body, &messageBody); err != nil { // Parse []byte to go struct pointer
		...
		result := email.SendEmail(to, messageBody.Subject, messageBody.Message)

		if result != nil {
			log.Println("Mail gönderilemedi")
		}
